...
#' Modern, Clean ggplot2 Theme with Roboto Font Family
#'
#' @description
#' A clean and modern theme for ggplot2 visualizations that uses the Roboto font family.
#' It builds upon theme_minimal() with customized typography, colors, and spacing.
#'
#' @param base_size Base font size for the theme (default: 11)
#'
#' @details
#' The theme features:
#' * White background with subtle light gray gridlines
#' * Custom color palette with navy blue titles and charcoal text
#' * Roboto font for body text and Roboto Slab for headers
#' * Carefully scaled text sizes and margins
...
theme_shubham <- function(base_size = 11) {
  ...
  theme_minimal(base_size = base_size) %+replace%
    ...
    theme(
      ...
      # Grid lines
      panel.grid.major = element_line(color = shubham_cols["light_gray"], size = 0.2),
      panel.grid.minor = element_line(color = shubham_cols["light_gray"], size = 0.1),

      # Text elements
      text = element_text(family = body_font, color = shubham_cols["charcoal"]),
      ...
      axis.ticks = element_line(color = shubham_cols["dark_gray"], size = 0.2),
      ...
      plot.margin = margin(t = base_size,
                           r = base_size,
                           b = base_size,
                           l = base_size)
    )
